			...
			<div class="p-3">
				<form action="{{ url()->current() }}" method="GET" class="max-w-280">
					<div class="input-group">
						...
						<div class="input-group-append search-submit"><button type="submit"><i class="tio-search"></i></button></div>
					</div>
				</form>
			</div>
						...
						<tr>
							...
							<td class="text-center">
								<div class="d-flex justify-content-center gap-2">
									...
									<a class="btn btn-outline-info btn-outline-info-dark icon-btn" title="{{ translate('view') }}" href="{{ route('vendor.orders.details', [$late['order_id']]) }}">
										<i class="fi fi-rr-eye"></i>
									</a>
									<a class="btn btn-outline-success btn-outline-success-dark icon-btn" target="_blank" title="{{ translate('invoice') }}" href="{{ route('vendor.orders.generate-invoice', [$late['order_id']]) }}">
										<i class="fi fi-rr-down-to-line"></i>
									</a>
									<button type="button" class="btn btn-outline-info btn-outline-info-dark icon-btn js-v-late-status" data-id="{{ $late['id'] }}" data-status="in_progress" title="{{ translate('in_progress') }}">
										<i class="fi fi-rr-time-forward"></i>
									</button>
									<button type="button" class="btn btn-outline-success btn-outline-success-dark icon-btn js-v-late-status" data-id="{{ $late['id'] }}" data-status="resolved" title="{{ translate('resolved') }}">
										<i class="fi fi-rr-check"></i>
									</button>
									<button type="button" class="btn btn-outline-danger btn-outline-danger-dark icon-btn js-v-late-status" data-id="{{ $late['id'] }}" data-status="rejected" title="{{ translate('rejected') }}">
...
@push('script')
<script>
    $(document).on('click', '.js-v-late-status', function () {
        const $btn = $(this);
        ...
        $.post({
            url: '{{ route('vendor.late-delivery.update-status') }}',
            data: payload
        }).done(function (res) {
