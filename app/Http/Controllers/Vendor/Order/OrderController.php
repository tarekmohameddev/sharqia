...
class OrderController extends BaseController
{
    ...
    public function bulkInvoices(Request $request)
    {
        ...
        // Governorates for city names (one query)
        ...
        // Vendor and config once (same for all vendor orders)
        $vendor = $this->vendorRepo->getFirstWhere(params: ['id' => $sellerId], relations: ['shop']);
        $companyPhone = getWebConfig(name: 'company_phone');
        $companyEmail = getWebConfig(name: 'company_email');
        $companyName = getWebConfig(name: 'company_name');
        $companyWebLogo = getWebConfig(name: 'company_web_logo');
        ...
        foreach ($ids as $oid) {
            ...
            try {
                ...
                \Illuminate\Support\Facades\Log::error('Vendor bulk invoice PDF failed for order', [
                    'order_id' => $oid,
                    'seller_id' => $sellerId,
        ...
        \App\Models\Order::whereIn('id', $printedIds)->where('seller_id', $sellerId)->update(['is_printed' => 1, 'order_status' => 'out_for_delivery']);
